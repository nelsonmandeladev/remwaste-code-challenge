export class SkipService {
  private static readonly SIZE_CATEGORIES: Record<
    SkipSizeCategory,
   ...
   */
  public static filterSkips(
    skips: Skip[],
    filter: SkipFilter,
  ): SkipFilterResult {
    const filteredSkips = skips.filter((skip) =>
      this.matchesFilter(skip, filter),
    );
  ...
  public static getFilterStats(skips: Skip[]): SkipFilterStats {
    return {
      ...
      availableAreas: [
        ...new Set(skips.map((skip) => skip.area).filter(Boolean)),
      ],
    };
  }
  ...
  private static matchesFilter(skip: Skip, filter: SkipFilter): boolean {
    ...
    if (filter.minSize !== undefined && skip.size < filter.minSize)
      return false;
    if (filter.maxSize !== undefined && skip.size > filter.maxSize)
      return false;
    if (
      filter.hirePeriodDays !== undefined &&
      skip.hirePeriodDays !== filter.hirePeriodDays
    )
      return false;

    const totalPrice = this.calculateTotalPrice(skip);
    if (filter.minPrice !== undefined && totalPrice < filter.minPrice)
      return false;
    if (filter.maxPrice !== undefined && totalPrice > filter.maxPrice)
      return false;

    if (filter.postcode !== undefined && skip.postcode !== filter.postcode)
      return false;
    if (filter.area !== undefined && skip.area !== filter.area) return false;
    if (filter.forbidden !== undefined && skip.forbidden !== filter.forbidden)
      return false;
    if (
      filter.allowedOnRoad !== undefined &&
      skip.allowedOnRoad !== filter.allowedOnRoad
    )
      return false;
    if (
      filter.allowsHeavyWaste !== undefined &&
      skip.allowsHeavyWaste !== filter.allowsHeavyWaste
    )
      return false;
    ...
    if (filter.createdAt) {
      ...
      if (filter.createdAt.from && createdDate < filter.createdAt.from)
        return false;
      if (filter.createdAt.to && createdDate > filter.createdAt.to)
        return false;
    }

    if (filter.updatedAt) {
      const updatedDate = new Date(skip.updatedAt);
      if (filter.updatedAt.from && updatedDate < filter.updatedAt.from)
        return false;
      if (filter.updatedAt.to && updatedDate > filter.updatedAt.to)
        return false;
    }
  ...
  private static getSizeCategoryCounts(
    skips: Skip[],
  ): Record<SkipSizeCategory, number> {
