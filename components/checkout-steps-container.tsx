 ...
 */
export function CheckoutStepsContainer({
  steps,
  activeIndex,
}: CheckoutStepsContainerProps): React.ReactElement {
  const { queryParams } = useGlobalUrlQueryParams();

  return (
    <div
      className={cn(
        "w-[83%] md:w-[96%] flex items-center bg-white/50 backdrop-blur-lg rounded-full p-4 overflow-x-auto transition-all duration-300",
        {
      ...
      {steps.map((step, index) => (
        <React.Fragment key={index}>
          ...
          {index < steps.length - 1 && (
            <div
              className={`flex-auto border-t ${index < activeIndex ? "border-blue-500" : "border-gray-300"} mx-4`}
            ></div>
          )}
        </React.Fragment>
      ))}
